...
def run_test():
    db = SessionLocal()
    print("Testing Member Exam Taking Flow API...")
    ...
    member = User(
        email="test_member_take@example.com",
        password=get_password_hash("testpassword"),
        fullname="Test Member Take",
        role="USER",
        status="ACTIVE"
    )
    ...
    try:
        ...
        # Add Member as APPROVED member
        g_member = GroupMember(group_id=group.id, user_id=member.id, role_in_group="MEMBER", status="APPROVED", joined_at=datetime.utcnow())
        db.add(g_member)
        db.commit()
        ...
        # Create ExamAssignee
        assignee = ExamAssignee(exam_id=exam.id, user_id=member.id, status="PENDING")
        db.add(assignee)
        db.commit()
        db.refresh(assignee)
        ...
        # TEST 8: Test Abandon Flow (Create another active exam and member abandons it)
        ...
        assignee_ab = ExamAssignee(exam_id=exam_ab.id, user_id=member.id, status="PENDING")
        db.add(assignee_ab)
        db.commit()
        db.refresh(assignee_ab)

        # Member starts exam_ab
        ...
        # Member abandons exam_ab (Exited/Close tab)
        ...
        # Member try to start again (Expected to fail)
        try:
        ...
        print("\nALL MEMBER EXAM TAKING TESTS PASSED SUCCESSFULLY!")
        ...
        db.query(ExamAnswer).filter(ExamAnswer.exam_assignee_id.in_(
            db.query(ExamAssignee.id).filter(ExamAssignee.user_id == member.id)
