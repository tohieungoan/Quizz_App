...
export function QuestionList({ questions, onEdit, onDuplicate, onDelete }: QuestionListProps) {
  return (
    <div id="questions-list-section" className="shrink-0 max-w-5xl mx-auto flex flex-col w-full mt-10 mb-12">
      ...
      {questions.length === 0 ? (
        <div className="text-center py-12 bg-surface-container-lowest border border-dashed border-outline-variant/50 rounded-2xl text-on-surface-variant text-sm shadow-sm">No questions yet. Start building your quiz manually or use AI to generate them!</div>
      ) : (
                ...
                {questions.map((question, index) => (
                  <tr key={question.id} className="group hover:bg-surface-bright">
                    ...
                    <td className="px-6 py-4 text-center"><div className="flex items-center justify-center gap-2">
                      ...
                      <button onClick={() => onDuplicate(question)} className="p-1.5 text-on-surface-variant hover:text-primary rounded-md transition-colors hover:bg-surface-container" title="Duplicate"><CopyPlus className="w-4 h-4" /></button>
