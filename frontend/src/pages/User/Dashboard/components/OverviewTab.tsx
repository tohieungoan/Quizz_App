...
import { Award, Flame, CheckSquare, Play, TrendingUp, PieChart, BookOpen, Clock, Sparkles, ChevronDown, ChevronUp, ArrowRight, AlertCircle } from 'lucide-react';
...
export const OverviewTab: React.FC<OverviewTabProps> = ({ 
  ...
  return (
    <div className="space-y-6 sm:space-y-8 text-left">
      {/* 1. Banner */}
      <div className="bg-gradient-to-r from-primary via-primary/90 to-secondary rounded-2xl sm:rounded-3xl p-5 sm:p-8 text-white shadow-lg relative overflow-hidden">
        <div className="max-w-xl space-y-2.5 sm:space-y-3 relative z-10">
          <span className="bg-white/20 backdrop-blur-md text-white px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-[10px] sm:text-xs font-semibold uppercase tracking-wider">
            User Dashboard
          </span>
          ...
          <div className="pt-1.5 sm:pt-2 flex flex-wrap gap-2.5 sm:gap-3">
            <button
              onClick={onJoinRoom}
              className="w-full sm:w-auto bg-white text-primary px-5 py-2.5 rounded-xl font-bold text-xs sm:text-sm hover:bg-indigo-50 transition-all shadow-md active:scale-95 flex items-center justify-center gap-2"
            >
      ...
      {/* 2. Top Metric Cards (2x2 Grid on Mobile, 4-Col on Desktop) */}
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
        <div className="bg-white p-3.5 sm:p-6 rounded-xl sm:rounded-2xl border border-outline-variant/30 shadow-sm flex flex-col sm:flex-row items-start sm:items-center gap-2.5 sm:gap-4">
          <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-primary/10 text-primary flex items-center justify-center font-bold shrink-0">
            <Award className="w-5 h-5 sm:w-6 sm:h-6" />
          </div>
          <div className="min-w-0">
            <p className="text-[10px] sm:text-xs font-semibold text-on-surface-variant uppercase tracking-wider truncate">Average Score</p>
            <h3 className="text-lg sm:text-2xl font-extrabold text-on-surface truncate">{averageScoreFormatted}</h3>
            <span className="text-[9px] sm:text-[10px] font-bold text-green-600 flex items-center gap-0.5 mt-0.5 truncate">
              <TrendingUp className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> Real-time average
            </span>
          </div>
        </div>

        <div className="bg-white p-3.5 sm:p-6 rounded-xl sm:rounded-2xl border border-outline-variant/30 shadow-sm flex flex-col sm:flex-row items-start sm:items-center gap-2.5 sm:gap-4">
          <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-purple-100 text-purple-600 flex items-center justify-center font-bold shrink-0">
            <Sparkles className="w-5 h-5 sm:w-6 sm:h-6" />
          </div>
          <div className="min-w-0">
            <p className="text-[10px] sm:text-xs font-semibold text-on-surface-variant uppercase tracking-wider truncate">Points (EXP)</p>
            <h3 className="text-lg sm:text-2xl font-extrabold text-on-surface truncate">{achievementPoints.toLocaleString()}</h3>
            <span className="text-[9px] sm:text-[10px] font-bold text-purple-600 mt-0.5 block truncate">Earned EXP & Rewards</span>
          </div>
        </div>

        <div className="bg-white p-3.5 sm:p-6 rounded-xl sm:rounded-2xl border border-outline-variant/30 shadow-sm flex flex-col sm:flex-row items-start sm:items-center gap-2.5 sm:gap-4">
          <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-green-100 text-green-600 flex items-center justify-center font-bold shrink-0">
            <CheckSquare className="w-5 h-5 sm:w-6 sm:h-6" />
          </div>
          <div className="min-w-0">
            <p className="text-[10px] sm:text-xs font-semibold text-on-surface-variant uppercase tracking-wider truncate">Completed</p>
            <h3 className="text-lg sm:text-2xl font-extrabold text-on-surface truncate">{completedExams.length} Exams</h3>
            <span className="text-[9px] sm:text-[10px] font-bold text-green-600 mt-0.5 block truncate">
              {assignedExams.length > 0 
                ? `${Math.round((completedExams.length / assignedExams.length) * 100)}% Done`
                : '100% Clear'}
            </span>
          </div>
        </div>
        ...
        <div className="bg-white p-3.5 sm:p-6 rounded-xl sm:rounded-2xl border border-outline-variant/30 shadow-sm flex flex-col sm:flex-row items-start sm:items-center gap-2.5 sm:gap-4">
          <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-orange-100 text-orange-600 flex items-center justify-center font-bold shrink-0">
            <Flame className="w-5 h-5 sm:w-6 sm:h-6" />
          </div>
          <div className="min-w-0">
            <p className="text-[10px] sm:text-xs font-semibold text-on-surface-variant uppercase tracking-wider truncate">Streak</p>
            <h3 className="text-lg sm:text-2xl font-extrabold text-on-surface truncate">{studyStreakDays} Days</h3>
            <span className={`text-[9px] sm:text-[10px] font-bold flex items-center gap-0.5 mt-0.5 truncate ${studyStreakDays > 0 ? 'text-orange-600' : 'text-slate-400'}`}>
              <Flame className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> {studyStreakDays > 0 ? 'Consecutive' : 'Streak Reset'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8">
        {/* Assigned Exams */}
        <div className="bg-white p-4 sm:p-6 rounded-2xl border border-outline-variant/30 shadow-sm space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="font-bold text-base sm:text-lg text-on-surface">Assigned Exams</h3>
            <span className="text-[11px] sm:text-xs font-semibold text-primary px-2.5 py-0.5 rounded-full bg-primary/10">{pendingExams.length} Pending</span>
          </div>
            ...
            {isLoadingExams ? (
              ...
              (showAllPending ? pendingExams : pendingExams.slice(0, 4)).map((exam) => {
                ...
                return (
                  <div
                    key={exam.id}
                    className="p-3 sm:p-4 rounded-xl border border-outline-variant/40 hover:border-primary/40 transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 sm:gap-4 bg-surface-container-lowest"
                  >
                    <div className="min-w-0 flex-1">
                      <h4 className="font-bold text-xs sm:text-sm text-on-surface truncate">{exam.title}</h4>
                    ...
                    {isOverdue ? (
                      <button
                        disabled
                        className="w-full sm:w-auto bg-slate-200 text-slate-500 text-xs font-bold px-3 py-1.5 rounded-lg cursor-not-allowed opacity-80 shrink-0 text-center border border-slate-300 flex items-center justify-center gap-1"
                      >
                      ...
                      <button
                        ...
                        className="w-full sm:w-auto bg-primary text-white text-xs font-bold px-4 py-2 rounded-lg hover:bg-primary/90 transition-all shrink-0 text-center"
                      >
        ...
        <div className="bg-white p-4 sm:p-6 rounded-2xl border border-outline-variant/30 shadow-sm space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="font-bold text-base sm:text-lg text-on-surface">Recent Activity</h3>
          </div>
          ...
          <div className="space-y-2.5 sm:space-y-3">
            {recentActivities.length === 0 ? (
              ...
              (showAllRecent ? recentActivities : recentActivities.slice(0, 4)).map((act) => (
                <div
                  key={act.id}
                  className="p-3 sm:p-4 rounded-xl border border-outline-variant/30 flex items-center justify-between gap-3 bg-surface-container-lowest"
                >
                  <div className="min-w-0 flex-1">
                    <h4 className="font-bold text-xs sm:text-sm text-on-surface truncate">{act.name}</h4>
                    <p className="text-[11px] sm:text-xs text-on-surface-variant mt-0.5 truncate">
                      {act.type} • {act.date}
                    </p>
                  </div>
                  <span
                    className={`text-xs sm:text-sm font-extrabold px-2.5 py-1 rounded-full shrink-0 ${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-8">
        ...
        <div className="lg:col-span-6 bg-white p-4 sm:p-6 rounded-2xl border border-outline-variant/30 shadow-sm space-y-4 sm:space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              <h3 className="font-bold text-base sm:text-lg text-on-surface flex items-center gap-2">
                <PieChart className="w-4 h-4 sm:w-5 sm:h-5 text-primary" /> Quiz & Activity Breakdown
              </h3>
              <p className="text-[11px] sm:text-xs text-on-surface-variant">Distribution of created, taken, and hosted quizzes</p>
            </div>
            <span className="self-start sm:self-auto text-[10px] sm:text-xs font-bold px-2.5 py-1 bg-surface-container rounded-lg text-on-surface-variant">
              Total: {totalActivities} Items
            </span>
          </div>

          <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-8 pt-1 sm:pt-2">
            ...
            <div className="relative w-36 h-36 sm:w-44 sm:h-44 shrink-0 flex items-center justify-center">
              ...
              <div className="absolute flex flex-col items-center justify-center text-center">
                <span className="text-xl sm:text-2xl font-black text-on-surface">{totalActivities}</span>
                <span className="text-[9px] sm:text-[10px] font-bold text-on-surface-variant uppercase tracking-wider">Activities</span>
              </div>
            </div>

            {/* Legend List */}
            <div className="space-y-2 sm:space-y-3.5 flex-1 w-full">
              {quizDistribution.map((item, i) => (
                <div key={i} className="flex items-center justify-between p-2 sm:p-2.5 rounded-xl bg-surface-container-lowest border border-outline-variant/20">
                  <div className="flex items-center gap-2">
                    <span className={`w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full ${item.color}`} />
                    <span className="text-[11px] sm:text-xs font-bold text-on-surface">{item.label}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-[11px] sm:text-xs font-extrabold text-on-surface">{item.count}</span>
                    <span className="text-[9px] sm:text-[10px] text-on-surface-variant ml-1 font-semibold">({item.percentage}%)</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="lg:col-span-6 bg-white p-4 sm:p-6 rounded-2xl border border-outline-variant/30 shadow-sm space-y-4 sm:space-y-5">
          <div>
            <h3 className="font-bold text-base sm:text-lg text-on-surface flex items-center gap-2">
              <BookOpen className="w-4 h-4 sm:w-5 sm:h-5 text-secondary" /> Subject Proficiency
            </h3>
            <p className="text-[11px] sm:text-xs text-on-surface-variant">Mastery level across enrolled subjects</p>
          </div>
                ...
                <div key={i} className="space-y-1 sm:space-y-1.5">
                  <div className="flex items-center justify-between text-[11px] sm:text-xs">
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2">
                      <span className="text-[9px] sm:text-[10px] font-extrabold px-1.5 py-0.5 rounded bg-surface-container text-on-surface-variant">
                        {sub.level}
                      </span>
